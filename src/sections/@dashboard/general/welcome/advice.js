...
export default function Advice() {
  ...
  // Define a state variable to track when a new discussion has been saved
  ...
  // decode the question from the base64 string
  useEffect(() => {
  ...
  useEffect(() => {
    async function generateDiscussion() {
      ...
      console.log('credits remained', remainingCredits)
      console.log('credits', remainingCredits)
      console.log('boardrooms', querySnapshot.size) 
  ...
  //     setLoading(true);
  //     const boardRoomRef = collection(db, 'users', user.uid, 'boardRooms');
  //     const querySnapshot = await getDocs(boardRoomRef);
  ...
  //     const docSnapshot = querySnapshot.docs[querySnapshot.docs.length - 1];
  //     const docData = docSnapshot.data();
  //     setDiscussion(docData.discussion);
  //     setLoading(false);
  //   }    
